...
class Property(models.Model):
    ...
    lead_count = fields.Integer(
        string='Lead Count', compute='_compute_lead_count')

    def _compute_lead_count(self):
        """Compute the number of CRM leads interested in this property."""
        for rec in self:
            rec.lead_count = self.env['crm.lead'].search_count(
                [('property_ids', 'in', rec.id)])

    def action_view_leads(self):
        """View all CRM leads for this property."""
        self.ensure_one()
        return {
            'name': _('Leads'),
            'view_mode': 'tree,form',
            ...
            'context': {'default_property_ids': [self.id]},
        }
