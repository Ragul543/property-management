...
class PropertyAuction(models.Model):
    ...
    def action_end(self):
        if self.participant_ids:
            ...
            now_dt = fields.Datetime.now()  # this is already a datetime object
            
            # Format datetime to MM/DD/YYYY HH:MM:SS
            formatted_now = now_dt.strftime('%m/%d/%Y %H:%M:%S')
            print("formatted now", formatted_now)
            
            # Assign ISO formatted string to end_time (which is correct type for fields.Datetime)
            self.end_time = now_dt.strftime('%Y-%m-%d %H:%M:%S')
