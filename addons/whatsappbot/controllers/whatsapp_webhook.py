        ...
        try:
            ...
            with request.env.registry.cursor() as new_cr:
                ...
                if event == 'message_received':
                    ...
                    _logger.info(f"[WEBHOOK DEBUG] Message data: from={message_data.get('from')}, to={message_data.get('to')}, body={message_data.get('body', '')[:50]}")

                    # Extract owner_phone from 'to' field (the logged-in WhatsApp account)
                    owner_phone = message_data.get('to', '').replace('@c.us', '')
                    if owner_phone:
                        # Update config with logged_in_phone if not already set
                        config = new_env['whatsapp.config'].sudo().search([('active', '=', True)], limit=1)
                        if config:
                            normalized_owner = new_env['whatsapp.conversation'].sudo()._normalize_phone(owner_phone)
                            if normalized_owner and config.logged_in_phone != normalized_owner:
                                config.logged_in_phone = normalized_owner
                                _logger.info(f"[WEBHOOK DEBUG] Updated config logged_in_phone to: {normalized_owner}")
