...
class WhatsAppConversation(models.Model):
    ...
    owner_phone = fields.Char(
        string='Owner Phone',
        index=True,
        help='Phone number of the logged-in WhatsApp account that owns this conversation'
    )
    ...
    mute_until = fields.Datetime(string='Muted Until')
    wallpaper = fields.Char(string='Chat Wallpaper')
    active = fields.Boolean(default=True)

    _sql_constraints = [
        ('phone_group_owner_unique', 'unique(phone, group_id, owner_phone)', 'A conversation with this identifier already exists for this WhatsApp account!'),
    ]
    ...
    @api.model
    def get_or_create(self, phone, partner_id=False, owner_phone=False):
        """Get existing conversation or create new one (for individual chats)

        Args:
            phone: The other party's phone number
            partner_id: Optional partner ID to link
            owner_phone: The logged-in WhatsApp account phone number
        """
        normalized_phone = self._normalize_phone(phone)
        if not normalized_phone:
            return self.browse()

        # Normalize owner_phone if provided
        normalized_owner = self._normalize_phone(owner_phone) if owner_phone else False

        # Try exact match first (individual chat) - also match owner_phone if provided
        domain = [
            ('phone', '=', normalized_phone),
            ('is_group', '=', False)
        ]
        if normalized_owner:
            domain.append(('owner_phone', '=', normalized_owner))

        conversation = self.search(domain, limit=1)

        # If not found with owner_phone, try without (for backwards compatibility)
        if not conversation and normalized_owner:
            ...
            if conversation:
                # Update existing conversation with owner_phone
                conversation.owner_phone = normalized_owner
        ...
        if not conversation and len(normalized_phone) >= 10:
            ...
            legacy_domain = [('is_group', '=', False)]
            if normalized_owner:
                legacy_domain.append(('owner_phone', '=', normalized_owner))
            all_convs = self.search(legacy_domain)
        ...
        if not conversation:
            vals = {
                ...
                'is_group': False,
                'owner_phone': normalized_owner,
            }
            conversation = self.create(vals)
            _logger.info(f"Created new WhatsApp conversation for {normalized_phone} (owner: {normalized_owner})")
    ...
    @api.model
    def get_or_create_group(self, group_id, group_name=False, participants=None, created_by_uid=False, owner_phone=False):
        ...
        Args:
            ...
            created_by_uid: Odoo user ID who created the group
            owner_phone: The logged-in WhatsApp account phone number
        """
        import json
        if not group_id:
        ...
        # Try exact match by group_id and owner_phone
        domain = [
            ('group_id', '=', group_id),
            ('is_group', '=', True)
        ]
        ...
        if not conversation:
            ...
            conversation = self.create(vals)
            _logger.info(f"Created new WhatsApp group conversation: {group_name} ({group_id}) by user {vals.get('created_by_uid')} (owner: {normalized_owner})")
        else:
    ...
    def action_mute(self, duration=None):
        """Toggle mute status or mute for specific duration

        Args:
            duration: 'forever', '8hours', '1week', or None to toggle
        """
        self.ensure_one()
        if duration is None:
            self.is_muted = not self.is_muted
            self.mute_until = False
        elif duration == 'forever':
            self.is_muted = True
            self.mute_until = False
        elif duration == '8hours':
            self.is_muted = True
            self.mute_until = fields.Datetime.now() + timedelta(hours=8)
        elif duration == '1week':
            self.is_muted = True
            self.mute_until = fields.Datetime.now() + timedelta(days=7)
        elif duration == 'unmute':
            self.is_muted = False
            self.mute_until = False
    ...
    def action_clear_chat(self):
        ...
        self.unread_count = 0
        self.last_message_body = False
        self.last_message_date = False
        self.last_message_direction = False
        self.last_message_author = False
    ...
    def _format_for_frontend(self):
        ...
        try:
            ...
            return {
                ...
                'mute_until': self.mute_until.isoformat() if self.mute_until else False,
    ...
    @api.model
    def get_conversations_for_user(self, limit=50, offset=0, owner_phone=False):
        """Get all conversations for current user (excluding archived)

        Args:
            limit: Maximum number of conversations to return
            offset: Pagination offset
            owner_phone: Filter by logged-in WhatsApp account phone number
        """
        domain = [('active', '=', True), ('is_archived', '=', False)]

        # Filter by owner_phone if provided
        if owner_phone:
            normalized_owner = self._normalize_phone(owner_phone)
            if normalized_owner:
                domain.append(('owner_phone', '=', normalized_owner))
