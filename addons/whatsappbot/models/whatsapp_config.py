...
class WhatsAppConfig(models.Model):
    ...
    logged_in_phone = fields.Char(
        string='Logged In Phone',
        help='Phone number of the currently logged-in WhatsApp account',
        readonly=True
    )
    ...
    def action_logout(self):
        ...
        try:
            ...
            if data.get('ok'):
                self.state = 'disconnected'
                self.logged_in_phone = False  # Clear logged in phone on logout
                self.last_check = fields.Datetime.now()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'display_notification',
    ...
    def send_message(self, recipient, message, is_group=False, quoted_message_id=None):
        ...
        Args:
            ...
            quoted_message_id: Optional WhatsApp message ID to reply to (quoted reply)
        """
        self.ensure_one()
        ...
        try:
            ...
            _logger.info(f"Sending message to {chat_id}: {message[:50]}...")
            ...
            if response.status_code != 200:
                _logger.error(f"WhatsApp server returned status {response.status_code}")
                raise UserError(f"WhatsApp server error (status {response.status_code})")

            data = response.json()

            if not data.get('ok'):
                error_msg = data.get('error', 'Failed to send message')
                _logger.error(f"WhatsApp send failed: {error_msg}")
                raise UserError(error_msg)

            _logger.info(f"Message sent successfully, ID: {data.get('messageId')}")
            return data

        except requests.exceptions.ConnectionError:
            _logger.error(f"Cannot connect to WhatsApp bot server at {self.server_url}")
            raise UserError(
                f"Cannot connect to WhatsApp bot server at {self.server_url}. "
                "Please make sure the bot server is running. "
                "Run: cd /opt/tr22/addons/whatsappbot/bot_server && ./start.sh"
            )
        except requests.exceptions.Timeout:
            _logger.error("WhatsApp bot server request timeout")
            raise UserError("WhatsApp bot server is not responding. Please check if it's running.")
        except requests.exceptions.RequestException as e:
            _logger.error(f"WhatsApp request error: {e}")
            raise UserError(f"Failed to communicate with WhatsApp server: {str(e)}")
