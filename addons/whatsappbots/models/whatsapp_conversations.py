...
class WhatsAppConversation(models.Model):
    ...
    def update_last_message(self, message):
        ...
        # Determine the last message preview text
        preview_text = (message.body or '')[:100]
        if not preview_text and message.has_media:
            # Show media type placeholder when message body is empty but has media
            media_labels = {
                'image': 'Image',
                'video': 'Video',
                'audio': 'Audio',
            ...
            preview_text = media_labels.get(message.message_type, 'Media')
    ...
    @api.model
    def fix_last_message_previews(self):
        """Fix last message previews for all conversations.
        This recalculates the last_message_body for conversations where
        the last message is a media message without text.
        """
        Message = self.env['whatsapp.message'].sudo()
        conversations = self.sudo().search([('active', '=', True)])
        ...
        for conv in conversations:
            ...
            if last_msg:
                # Recalculate the preview text
                preview_text = (last_msg.body or '')[:100]
                if not preview_text and last_msg.has_media:
                    media_labels = {
                        'image': 'Image',
                        'video': 'Video',
                        'audio': 'Audio',
                    ...
                    preview_text = media_labels.get(last_msg.message_type, 'Media')

                # Update if different or empty
                if preview_text and conv.last_message_body != preview_text:
                    conv.last_message_body = preview_text
                    conv.last_message_date = last_msg.timestamp
                    conv.last_message_direction = last_msg.direction
                    fixed_count += 1
                    _logger.info(f"Fixed conversation {conv.id}: last_message='{preview_text}'")

        _logger.info(f"Fixed {fixed_count} conversation last message previews")
        return fixed_count
