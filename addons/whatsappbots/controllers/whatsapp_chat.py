...
class WhatsAppChatController(http.Controller):
    ...
    @http.route('/whatsapp/fix_previews', type='http', auth='none', csrf=False, methods=['POST', 'GET'])
    def fix_message_previews(self, **kwargs):
        """Fix last message previews for all conversations (fixes 'No messages' issue)"""
        try:
            fixed_count = request.env['whatsapp.conversation'].sudo().fix_last_message_previews()
            return json.dumps({'ok': True, 'fixed_count': fixed_count})
        except Exception as e:
            _logger.exception(f"Error fixing message previews: {e}")
            return json.dumps({'ok': False, 'error': str(e)})
    ...
    def get_whatsapp_status(self, **kwargs):
        """Get current WhatsApp connection status"""
        try:
            config = request.env['whatsapp.config'].get_default_config()
            config.action_check_status()
            return {
                'ok': True,
                'status': config.state,
            ...
            _logger.exception(f"Error checking WhatsApp status: {e}")
            return {'ok': False, 'error': str(e), 'status': 'disconnected', 'connected': False}
    ...
    def get_qr_code(self, **kwargs):
        ...
        try:
            ...
            if data.get('qr'):
                ...
                return {'ok': False, 'error': data.get('error', 'QR code not available')}
